...
@Component({
  selector: 'app-research-coponent',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './research-coponent.html',
  styleUrls: ['./research-coponent.css']
})
export class ResearchCoponent implements OnInit {
  ads: any[] = [];
  categories: any[] = [];
  cities: any[] = [];
  loading = true;
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      // Récupérer le paramètre 'q' pour la recherche
  ...
  loadAds(): void {
    ...
    // Utiliser le paramètre 'search' pour rechercher dans le titre
    if (this.searchQuery) {
    ...
    this.annonceService.getAds(params).subscribe({
      next: (response) => {
        ...
        // Si une recherche est active, filtrer UNIQUEMENT par le titre
        if (this.searchQuery && this.searchQuery.trim()) {
          const searchTerm = this.searchQuery.toLowerCase().trim();
          this.ads = allAds.filter((ad: any) =>
            ad.title && ad.title.toLowerCase().includes(searchTerm)
          );
          console.log(`Recherche "${this.searchQuery}": ${this.ads.length} résultat(s) trouvé(s)`);
        } else {
          this.ads = allAds;
        }
